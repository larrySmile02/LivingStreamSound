...
public class SQLiteDatabase implements Serializable {
    ...
    // 数据库默认设置
    private final static String DB_NAME = "lecainew.db"; // 默认数据库名字
     ...
     * @param params  数据参数信息
     */
    public SQLiteDatabase(Context context, DBParams params, DBUpdateListener tadbUpdateListener) {
        this.mContext = context;
        this.mDatabaseHelper = new DBHelper(context, params.getDbName(),
...
//                Log.e("该主键ID已存在");
     ...
     */
    public Boolean insert(Object entity, com.md.basedpc.persistence.db.ArrayList updateFields) {
        if (!hasTable(entity.getClass())) {
            creatTable(entity.getClass());
        }
        SqlBuilder getSqlBuilder = SqlBuilderFactory.getInstance()
        ...
        try {
            Log.d("插入:" + getSqlBuilder.getSqlStatement());
        } catch (DBException e) {
            Log.e(e.getMessage());
        } catch (IllegalAccessException e) {
            Log.e(e.getMessage());
        }
        return execute(getSqlBuilder, entity);
    }
